...
echo"<div class='bd-example'>
	...
	<div class='card border-success'>
		...
		$stmt2 = $conn->query("SELECT id_categoria, SUM(nb_valor) nbvalor, SUM(valor_sum) valorsum, SUM(qtd_sum) qtdsum, SUM(nb_qtd) nbqtd FROM v_categoria_sums WHERE id_pedido = $pid AND id_categoria = $cid GROUP BY id_categoria");
		while($row2 = $stmt2->fetch(PDO::FETCH_OBJ)){
		
		$execpercent = 0;
		
		$execpercent = $row2->qtdsum / $row2->nbqtd;
		$execpercent = round($execpercent);
		
	
			$medpercent = 0;
		
		$medpercent = (int)$row2->valorsum / (int)$row2->nbvalor;
		$medpercent = round($medpercent);
		...
		echo"
			<div class='col-md-4 h5'>
				<div class='callout callout-info'>
				  ...
				  <strong class='h6'>".$execpercent."% - (".$row2->qtdsum."/".$row2->nbqtd.")</strong>
			</div>
		</div>
			<div class='col-md-4 h5'>
...
<!-- Modal Nova Medição  -->
	<div class="modal" style="text-align: left" id="modalMedi" tabindex="-1" role="dialog" aria-labelledby="modalMedi" aria-hidden="true">
						  <div class="modal-dialog" role="document">
							<div class="modal-content">
							  <div class="modal-header">
								<h5 class="modal-title" id="modalMedi"><?php echo "Pedido: ".$row3->tx_codigo." - ".$mid."ª Medição";?></h5>
								<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								  <span aria-hidden="true">&times;</span>
								</button>
							  </div>
    ...
    <div class="form-row">			
	  <div class="form-group col-md-8">
		<label for="formAtividade">Data </label>
		<input style="text-transform: uppercase;" type="text" class="form-control" id="formAtividade" placeholder="Descrição Atividade" name="Atividade">
	  </div>
	  <div class="form-group col-md-4">
		<label for="formTubo">Tubo</label>
