...
function getItems($conn,$string){
    
    try{
    $stmt = $conn->query("SELECT itm.*, idx.*, est.nb_vendidas, est.nb_disponivel FROM item AS itm
                        INNER JOIN indice AS idx ON itm.id_indice = idx.id_indice
                        LEFT JOIN fornecedor AS frn ON itm.id_fornecedor = frn.id_fornecedor
                        LEFT JOIN estoque AS est ON itm.tx_codigo LIKE est.tx_codigo
                        WHERE itm.tx_codigo LIKE '%".$string."%';");
                            
    $data = $stmt->fetchAll(PDO::FETCH_OBJ);
    
    }catch(PDOException $e)
    {
    echo "Erro ao carregar coleção! " . $e->getMessage();
    }
    return $data;
}

function getCollection($conn,$string){

    $data = getItems($conn,$string);
    
    $collection = array();

    foreach($data as $item){
        $color = (int)$item->id_indice;
        $color = cat_color($color);

        $piece = array();
        $price = 1.00 * ($item->nb_valor * $item->nb_prazo);
        $piece['color'] = $color;
        $piece['codigoItem'] = $item->tx_codigo;
        $piece['tipoItem'] = $item->tx_tipo;
        $piece['descricaoItem'] = $item->tx_descricao;
        $piece['tipoIndice'] = $item->tx_indice;
        $piece['valorIndice'] = $item->nb_prazo;
        $piece['valorItem'] = $item->nb_valor;
        $piece['valorFinal'] = $price;
        $piece['dataItem'] = data_usqlS($item->dt_item);
        $piece['qtdEstoque'] = $item->nb_disponivel;
        if($piece['qtdEstoque'] == null) $piece['qtdEstoque'] = 0;
        array_push($collection, $piece);

    }

    header('Content-type:application/json');
        $output = json_encode($collection);
        echo $output;
}

function getItem($conn,$arg){
    
    $stmt = $conn->prepare("SELECT * FROM item AS itm
                        INNER JOIN indice AS idx ON itm.id_indice = idx.id_indice
                        INNER JOIN fornecedor AS frn ON itm.id_fornecedor = frn.id_fornecedor
                        WHERE itm.tx_codigo LIKE :tx_codigo
                        LIMIT 1");
    $stmt->bindParam(':tx_codigo', $arg);  
    $stmt->execute();
...
function getPeca($conn,$arg){

    $item = getItem($conn,$arg);
